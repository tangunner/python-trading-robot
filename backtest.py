...
class PaperTradingAccount(object):
    ...
    def setBacktestDates(self, historical_prices, start=None, end=None):
        if start and end:
            self.backtestStartDate = start
            self.backtestEndDate = end
        else:
...
if backtesting:
    ...
    # Grab historical prices with the desired frequency
    historical_prices = trading_robot.grab_historical_prices(
        start=start_date,
        end=end_date,
        frequency=1,
    ...
    if not historical_prices['aggregated']:
        raise Exception('Bad start or end date for historical quotes. No data returned.')

    account.setBacktestDates(
        historical_prices, 
        start = start_date, 
        end = end_date
        )
...
discount_ratios = [0.90, 0.80, 0.70, 0.60, 0.50]   # 1 - pct discount
interval = 0.10
...
indicator_client.set_indicator_signal(
    ...
    buy=1.01,                      # set outside possible vals to restrict buys
    sell=1,
...
#############################
# LIVE PAPERTRADING SECTION #
#############################

elif not backtesting:
    
    while True:

        # Grab latest prices bar (uses the freq specified for hist quotes)
        latest_bars = trading_robot.get_latest_bar(symbols=[symbol])

        date = latest_bars[-1]['datetime']
        ...
        locked_indicators = trading_robot.portfolio.check_portfolio_indicators(
            symbol=latest_bars[-1]['symbol'])
        
        # Check for signals.
        signals = indicator_client.check_signals(locked_indicators = locked_indicators)
        ...
        if signals['sells'].notnull().any():
            if not signals['sells'].empty and trading_robot.portfolio.get_ownership_status(symbol):
                ...
                # grab the highest cost lot purchased
                first_lot = trading_robot.portfolio.position_lots[symbol][0]
                ...
                if first_lot['annualized_return'] < account.minReturn or \
                    ...
                    price = stock_frame['close'].item()
                    sma_premium = stock_frame['close_to_sma'].item()
                    total_quantity = int(trading_robot.portfolio.positions[symbol]['quantity'])
                    sale_quantity = round(account.saleSignalRatio * total_quantity, 0)
        ...
        if signals['buys'].notnull().any():
            if not signals['buys'].empty:
                ...
                price = stock_frame['close'].item()
                disc_ratio = stock_frame['discount_ratio'].item()
                quantity = int(account.lotSizing / price)
                cost_basis = price * quantity
